...
app.get('/micro_post/:id', function(req, res) {
	...
	db.get("select * from micro_posts where id = ?", parseInt(req.params.id), function(err, row) {
		...
		if (row.author_id > 0) {
			db.get("select name from authors where id = ?", parseInt(row.author_id), function(err, authRow) {
				if(err) {
					throw err;
				}
				res.render('show_micro_post.ejs', { thisMicroPost : row, author : authRow });
//				res.render('show_micro_post.ejs', { thisMicroPost : row, author : authRow, tags : tagRows, snippets : snippetRows } );
			});
		} else {
			res.render('show_micro_post.ejs', { thisMicroPost : row, author : { name : null } });
		}
    });
});
...
app.get('/micro_post/:id/edit', function(req, res) {
	db.get ("select * from micro_posts where id = ?;", parseInt(req.params.id), function(err, row) {
		...
		db.all("select * from authors", function (err, authorRows) {
			...
			res.render("edit_micro_post.ejs", { thisMicroPost : row, authors : authorRows });
		});
	});
});
